document.getElementById('formAluno')
  .addEventListener('submit', function (event) {
    ...
    //campo nível
    let nívelSelecionado = ''
    if (document.getElementById('nível-0').checked) {
      nívelSelecionadoSelecionado = 'Iniciante';
    } else if (document.getElementById('nível-1').checked) {
      nívelSelecionadoSelecionado = 'Intermediário';
    } else if (document.getElementById('nível-2').checked) {
      nívelSelecionadoSelecionado = 'Avançado';
    }
    ...
    const dadosAluno = {
      ...
      nível: nívelSelecionadoSelecionado
    }
    //testando...
    //alert(JSON.stringify(aulas))

    if (document.getElementById('id').value !== '') { //Se existir algo, iremos alterar,
      alterar(event, 'aulas', dadosAluno, document.getElementById('id').value)
    } else {
      incluir(event, 'aulas', dadosAluno)
    }
  })
...
async function incluir(event, collection, dados) {
  ...
  return await firebase.database().ref(collection).push(dados)
    .then(() => {
      alerta('✅Inscrição concluída, logo entraremos em contato!', 'success')
      document.getElementById('formAluno').reset()//limpa
    })
    .catch(error => {
      alerta('❌Falha ao concluir à inscrição: ' + error.message, 'danger')
    })
}
...
async function alterar(event, collection, dados, id) {
  ...
  return await firebase.database().ref().child(collection + '/' + id).update(dados)
    .then(() => {
      alerta('✅Inscrição alterada com sucesso!', 'success')
...
async function obtemAluno() {
  ...
  await firebase.database().ref('aulas').orderByChild('nome').on('value', (snapshot) => {
    ...
    snapshot.forEach(item => {
      ...
      novaLinha.insertCell().textContent = item.val().instrumentos
      novaLinha.insertCell().textContent = item.val().nível
      novaLinha.insertCell().innerHTML = `<button class='btn btn-sm btn-danger' title='Apaga o aluno selecionado' onclick=remover('${db}','${id}')> <i class='bi bi-trash'></i> </button>
                                          <button class='btn btn-sm btn-warning' title='Editar o aluno selecionado' onclick=carregaDadosAlteracao('${db}','${id}')> <i class='bi bi-pencil-square'></i> </button>`
    })
  })
...
async function remover(db, id) {
  if (window.confirm('⚠ Confirma a exclusão do cliente?')) {
    ...
    dadosExclusao.remove()
      .then(() => {
        alerta('✅Aula cancelada com sucesso!', 'success')
      })
      .catch(error => {
        alerta(`❌Falha ao cancelar a aula: ${error.message}`)
      })
  }
}
...
async function carregaDadosAlteracao(db, id) {
  await firebase.database().ref(db + '/' + id).on('value', (snapshot) => {
    ...
    document.getElementById('endereço').value = snapshot.val().email
    document.getElementById('telefone').value = snapshot.val().peso
    document.getElementById('instrumentos').value = snapshot.val().altura
    document.getElementById('nível').value = snapshot.val().nascimento
    if (snapshot.val().nível === 'Intermediário') {
      document.getElementById('nível-1').checked = true
    } else {
      document.getElementById('nível-0').checked = true
    }
  })
